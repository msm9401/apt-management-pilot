...
def hello_guest(request):
    ...
    """
    # 임시 로그인에 필요한 토큰 생성
    temp_token = AuthToken.objects.create(request.uset)[1]

    # 임시 유저의 아파트 설정 후 저장
    temp_house = Apartment.objects.filter(kapt_name__in=["용인신갈푸르지오", "분당 파크뷰"])
    list(map(lambda house: request.user.my_houses(house), temp_house))
    request.user.save()
...
class ApartmentList(APIView):
    ...
    ParseError 발생 시
    {
        "detail": "ParseError안에 정의한 문자열",
        "error": "config - exceptions.py에 정의한 문자열"
        "status_code": 400
    }
    """
    ...
    def get(self, request):
        ...
        if not max_apt_id:
            raise ParseError("아파트 정보를 불러올 수 없습니다.")  # 400
...
class SearchApartment(APIView):
    ...
    def get(self, request):
        if not request.query_params.get("keyword"):
            raise ParseError("검색어를 입력하세요.")  # 400
        if len(request.query_params.get("keyword")) == 1:
            raise ParseError("1글자 이상으로 검색하세요.")  # 400
        ...
        if not apt_searched:
            raise ParseError("찾으시는 결과가 없습니다.")  # 400
